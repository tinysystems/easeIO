...
void task_filter()
{
   ...
   while (samples < SIGNAL_LENGTH) {
       ...
       status = call_IO(msp_copy_q15(&copyParams, &input_lea[samples], &circularBuffer[copyindex]),"Always");

       status = call_IO(msp_fir_q15(msp_fir_q15(&firParams, &circularBuffer[filterIndex], &result[samples])),"Always");
