...
void task_sense()
{
    call_IO(capture(),"Single");

    IO_Block_begin("Single")
        temperature = call_IO(msp_sample_temperature(),"Timely", 10000);
        humidity = call_IO(msp_sample_temperature(),"Always");
    IO_Block_end()
...
void task_convol_mul()
{
    ...
    IO_Block_begin("Single")
       ...
       status = call_IO(msp_fir_q15(&firParams, &input_lea, &result),"Always")
    ...
    IO_Block_begin("Single")
       ...
       addParams.rows = 1;
       status = call_IO(msp_matrix_add_q15(&addParams, &input_lea, &biasBuffer, &result),"Always")
       msp_checkStatus(status);
...
void task_fc_mul()
{
    ...
    call_IO(status = msp_matrix_mpy_q15(&mpyParams, &input_lea, &biasBuffer, &result),"Always");
    msp_checkStatus(status);
