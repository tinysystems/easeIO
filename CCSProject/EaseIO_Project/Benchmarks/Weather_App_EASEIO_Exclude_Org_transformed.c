...
void task_convol_mul()
{
	...
	DMA_Data.DMA_Privatization[DMACounter-1] = COMPLETED;
       	...
       	DMA_Data.DMA_Control[DMACounter] = ALWAYS;
       	  DMA_Data.DMA_CheckLock[DMACounter] = SET;
       	EASEIO_DMA_copy( (uint32_t) &FILTER_COEFFS_EX1, (uint32_t) &filterCoeffs, sizeof(filterCoeffs),"Exclude");
	...
	DMA_Data.DMA_Privatization[DMACounter-1] = COMPLETED;
       	...
       	DMA_Data.DMA_Control[DMACounter] = ALWAYS;
       	  DMA_Data.DMA_CheckLock[DMACounter] = SET;
       	EASEIO_DMA_copy((uint32_t)(&conv2_wm+ (filter-1)*(IMAGE_LENGTH/2)), (uint32_t) &biasBuffer, IMAGE_LENGTH/2,"Exclude"); 
       	if(!DMA_Data.DMA_Privatization[DMACounter-1]){ 
       	DMA_Data.DMA_Privatization[DMACounter-1] = COMPLETED;
       	} 
       	 else {}
...
void task_fc_mul()
{
	...
	DMA_Data.DMA_Privatization[DMACounter-1] = COMPLETED;
    	...
    	DMA_Data.DMA_Control[DMACounter] = ALWAYS;
    	  DMA_Data.DMA_CheckLock[DMACounter] = SET;
    	EASEIO_DMA_copy((uint32_t)(&fc2_w + (fc_parse)*IMAGE_LENGTH + (fc_part-1)*(IMAGE_LENGTH/2)), (uint32_t) &biasBuffer, IMAGE_LENGTH/2,"Exclude"); 
    	if(!DMA_Data.DMA_Privatization[DMACounter-1]){ 
    	DMA_Data.DMA_Privatization[DMACounter-1] = COMPLETED;
    	} 
    	 else {}
